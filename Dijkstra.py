...
if __name__ == '__main__':
    myMaze = maze(10, 15)
    myMaze.CreateMaze(10,10, loopPercent=100)
    # myMaze.CreateMaze(loadMaze='dijkMaze.csv')

    h1 = agent(myMaze, color=COLOR.red)
   ...
   # h3.cost = 100
    ...
    path, c = dijkstra(myMaze, h1, start=(6, 1))
    textLabel(myMaze, 'Total Cost', c)

    # a=agent(myMaze,color=COLOR.cyan,filled=True,footprints=True)
    a = agent(myMaze, 6, 1, color=COLOR.red, filled=True, footprints=True, shape='arrow')
